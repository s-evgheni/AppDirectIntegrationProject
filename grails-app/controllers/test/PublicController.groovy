...
class PublicController implements InitializingBean {
    ...
    protected void authenticateAndRedirect(String username) {
        ...
        if (savedRequest && !config.successHandler.alwaysUseDefault) {
            ...
            redirect url: getRedirectUrl(username)
        }
    }

    private def getRedirectUrl(username){
        try{
            if(springSecurityService.userDetailsService.loadUserByUsername(username).authorities.toString().contains("ROLE_ADMIN"))
                return '/secured/admins'
            else if(springSecurityService.userDetailsService.loadUserByUsername(username).authorities.toString().contains("ROLE_USER"))
